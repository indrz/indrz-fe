$(function () {
  ...
  $('#jstree')
    .on('select_node.jstree', function (evt, data) {
      const nodeName = $('#jstree').jstree(true).get_text(data.node.id);
      const poiIconName2 = $('#jstree').jstree(true).get_icon(data.node.id);

      const poiName = nodeName.trim();
      ...
      if (poiExist(poiName)) {
        ...
        const Catid = data.node.id;
        const catNumId = Catid.split('_')[1];
        const poiIconName = $('#jstree').jstree(true).get_icon(data.node.id);
    ...
    .on('deselect_node.jstree', function (evt, data) {
      const nodeName = $('#jstree').jstree(true).get_text(data.node.id);
      const poiIconName3 = $('#jstree').jstree(true).get_icon(data.node.id);
