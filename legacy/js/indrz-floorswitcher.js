...
function setRouteStyle (layer) {
  ...
  for (let i = 0; i < features.length; i++) {
    const feature_floor = features[i].getProperties().floor;
...
function setLayerVisible (index) {
  ...
  if (switchableLayers.length > 0) {
    ...
    if (searchLayer) {
      const len_searchRes = searchLayer.getProperties().source.getFeatures().length;
      if (len_searchRes < 2 && len_searchRes !== 0) {
        // console.log("length search res " + len_searchRes)
        const floor_searchLayer = searchLayer.getProperties().source.getFeatures()[0].getProperties().floor_num;
        ...
        const sfeatures = searchLayer.getSource().getFeatures();
        for (let s = 0; s < sfeatures.length; s++) {
          let sfeature_floor = sfeatures[s].getProperties().floor_num;
          ...
          const geoType = sfeatures[s].getGeometry().getType();
