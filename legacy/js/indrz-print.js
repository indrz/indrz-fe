...
function getMapSize () {
  const mapSizePixels = map.getSize()[0] - map.getSize()[1];
  ...
  const xScreenDist = mapExtent[2] - mapExtent[0];
  // console.log("xScreenDist: " + xScreenDist);
  const pxPerMeter = map.getView().getResolution(); // resoltuion is is the size of 1 pixel in map units
  // console.log("px per meter: " + pxPerMeter);
  const scaleVal = mapWidthPixels * pxPerMeter;
  // console.log("scale val: " + scaleVal);

  const foorss = mapWidthPixels / 200;
  const newwidth = mapWidthPixels / foorss;
  // console.log("width in mm: " + newwidth);

  const newheight = mapHeightPixels / foorss;
...
function map2pdf () {
  map.once('postcompose', function (event) {
    ...
    const ratio_portrait = canvasMapWidth / canvasMapHeight;
    ...
    const mm_r = 72 / 25.4; // 2,833
    const px_r = 96 / 72; // 1,3333
    ...
    if (canvas.toBlob) {
      canvas.toBlob(
        function (blob) {
          ...
          let maxWidth = pdfWidth;
          let maxHeight = pdfHeight;
          ...
          const pdfMapHeight = pdfHeight - pdfTopMargin - pdfBottomMargin;

          const title_x_pos = pdfMapWidth / 2.4;
          const title_y_pos = 25;
          ...
          const x = calculateAspectRatioFit(canvasMapWidth, canvasMapHeight, maxWidth,
            maxHeight);
