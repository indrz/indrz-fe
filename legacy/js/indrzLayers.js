...
function createWmtsLayer (layerSrcName, type, isVisible, sourceName) {
  let gg, sm, templatepng, urlsbmappng, WmtsTileSource, wmtsLayer;
...
function createWmtsLayerIndrz (name, layerSrcName, floorNumber, isVisible, zIndexValue) {
  ...
  gg = ol.proj.get('EPSG:4326');
  sm = ol.proj.get('EPSG:3857');
  const gridsetName = 'EPSG:3857';
  const gridNames = ['EPSG:3857:0',
    'EPSG:3857:1',
    'EPSG:3857:2',
    'EPSG:3857:3',
    'EPSG:3857:4',
...
function createImageWms (geoserverLayer) {
  ...
  const wmsFloorLayer = new ol.layer.Image({
    source: wmsSource2
  });
...
function createTileXyz (name, layer, floor, is_visible, zIndex) {
  const f_url = baseGeoserverUrl + 'gwc/service/tms/1.0.0/' + layer + '/{z}/{x}/{-y}.png';

  const tmp_lyr = new ol.layer.Tile({
...
const grey_bmapat = createWmtsLayer('bmapgrau', '.png', true, 'basemap.at');
// var wmsOutdoorMap = createWmtsLayerIndrz('outdoorMap', 'indrz:outdoor', '0', 'true', 1 );
const ortho30cm_bmapat = createWmtsLayer('bmaporthofoto30cm', '.jpg', false, 'basemap.at');

const backgroundLayerGroup = new ol.layer.Group({ layers: [grey_bmapat, ortho30cm_bmapat], name: 'background maps' });

let wmsE00, wmsE01, wmsE02, wmsE03;
...
const wmsfloorLayerGroup = new ol.layer.Group({ layers: [wmsE00, wmsE01, wmsE02, wmsE03], name: 'wms floor maps' });
const poiLayerGroup = new ol.layer.Group({ layers: [], id: 99999, name: 'poi group' });
const campusLocationsGroup = new ol.layer.Group({ layers: [], id: 900, name: 'campus locations' });

const routeLayerGroup = new ol.layer.Group({ layers: [], name: 'route group' });

// var floor_layers = [ wmsUG01, wmsE00, wmsE01, wmsE02, wmsE03, wmsE04, wmsE05, wmsE06 ];

const OsmBackLayer = new ol.layer.Tile({
  source: new ol.source.OSM(),
  visible: true,
  type: 'background'
});
...
indrzApiCall(baseApiUrl + 'campus/1/floors/')
  .then(function (response) {
    ...
    for (let i = 0; i < floors_info.length; ++i) {
...
function appendFloorNav (floor_info, index) {
  ...
  if (index[0] === 1) {
    const option_html = "<option selected='selected' value='" + index + "'>" + floor_info + '</option>';
