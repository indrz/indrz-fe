...
const image = new ol.style.Icon(/** @type {olx.style.IconOptions} */ ({
  anchor: [0.5, 46],
...
const poiHideIcon = new ol.style.Icon(/** @type {olx.style.IconOptions} */ ({
  anchor: [0.5, 46],
  anchorXUnits: 'fraction',
  anchorYUnits: 'pixels',
  src: '/static/homepage/img/other.png',
...
const show_poi_search_style = new ol.style.Style({
  image: image
});

const hide_poi_search_style = new ol.style.Style({
  image: poiHideIcon
});

const show_search_style = new ol.style.Style({
...
const hide_search_style = new ol.style.Style({
  stroke: new ol.style.Stroke({
    color: '#fff6d2',
    width: 3
  }),
...
const styleFunctionHide = function (feature, resolution) {
  const fType = feature.getGeometry().getType();

  if (fType === 'MultiPolygon') {
    return styles[feature.getGeometry().getType()];
  } else if (fType === 'MultiPoint') {
    const poiImage = new ol.style.Icon(/** @type {olx.style.IconOptions} */ ({
      anchor: [0.5, 46],
      anchorXUnits: 'fraction',
      anchorYUnits: 'pixels',
    ...
    const stylesPoi = new ol.style.Style({
      image: poiImage
    });
...
const search_res_style = new ol.style.Style({
  fill: new ol.style.Fill({
...
function searchType (obj) {
  if (obj.length === 1) {
    const indrzSearchTypes = ['poi', 'space', 'building', 'campus', 'externalApi'];
...
function activateFloor (feature) {
  ...
  for (let i = 0; i < switchableLayers.length; i++) {
...
function searchIndrz (campusId, searchString, zoomLevel) {
  const searchUrl = hostUrl + req_locale + '/search/' + searchString + '?format=json';
  ...
  indrzApiCall(searchUrl).then(function (response) {
    ...
    searchSource.forEachFeature(function (feature) {
      const f_name = feature.get('name');

      const f_extent = feature.getGeometry().getExtent();
      const f_center = ol.extent.getCenter(f_extent);
      const zoom_coord = [f_center];
      ...
      const featureNameGet = 'category_' + req_locale;
    ...
    const centerCoord = ol.extent.getCenter(searchSource.getExtent());
    ...
    if (featuresSearch.length === 1) {
      ...
      const htmlInsert = '<p href="#" class="list-group-item indrz-search-res" > ' + gettext('Sorry nothing found') + '</p>';
      $('#search-results-list').append(htmlInsert);
    } else {
...
function searchForRoute (startSearchText, endSearchText) {
  ...
  $.when(fetcherMd(startSearchText), fetcherMd(endSearchText)).then(function (startPoint, endPoint) {
    ...
    let startPointProcessed = getCoordinatesFromGlobalRouteInfo('start');
    ...
    let endPointProcessed = getCoordinatesFromGlobalRouteInfo('end');
    ...
    let startPointText, endPointText;
...
function getCoordinatesFromResult (result) {
  ...
  if (
    ...
    let coordinates = result[0].features[0].properties.centerGeometry.coordinates;
...
function showRes (featureName, x, y, poiId) {
  ...
  searchLayer.getSource().forEachFeature(function (feature) {
    const f_properties = feature.getProperties();
...
const fetchData2 = function (text) {
