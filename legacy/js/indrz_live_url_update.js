...
function update_url (mode) {
  const current_extent2 = map.getView().calculateExtent(map.getSize());
  const current_zoom2 = map.getView().getZoom();
  const center_crd = map.getView().getCenter();
  const centerx2 = center_crd[0];
  const centery2 = center_crd[1];

  let url = '/?campus=' + building_id + '&centerx=' + centerx2 + '&centery=' + centery2 +
