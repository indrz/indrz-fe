const popup_container = document.getElementById('indrz-popup');
const popup_container_plain = document.getElementById('indrz-popup-plain');
const popup_content = document.getElementById('popup-content');
const popup_links = document.getElementById('popup-links');
const popup_closer = document.getElementById('popup-closer');

const selectRoom = new ol.interaction.Select({
 ...
 */
const popup_overlay = new ol.Overlay(/** @type {olx.OverlayOptions} */ ({
...
function closeIndrzPopup () {
  for (const member in globalPopupInfo) { globalPopupInfo[member] = null; }
  for (const st in globalSearchInfo) { globalSearchInfo[st] = false; }
...
function getRoomInfo (floor) {
  ...
  const availableWmsLayers = [wmsE00, wmsE01, wmsE02, wmsE03];
  let newel;
...
const displayFeatureInfo = function (pixel) {
  let feature = map.getFeaturesAtPixel(pixel);
  ...
  if (feature) {
    ...
    const featuresWms = map.getFeaturesAtPixel(pixel)
    ...
    const url = wmsSource2.getGetFeatureInfoUrl(
...
let routeToValTemp = '';
let routeToPoiIdTemp = '';
let routeFromValTemp = '';
let routeFromPoiIdTemp = '';
let objCenterCoords = '';
let floorNum;
...
function open_popup (properties, coordinate, feature, offsetArray) {
  ...
  const name_popup = properties.name

  const indrzLibraryShelf = ''
  ...
  let titlePopup = '';
  const titleBuildingName = gettext('Building: ');
  const titleFloorNumber = gettext('Floor Number: ');
  const titleRoomcode = gettext('Room Number: ');
  const titleRoomCat = gettext('Category: ');
  const titleFrontOffice = gettext('Front Office: ');
  const titleNearestEntrance = gettext('Entrance: ');
  const titleHomepage = gettext('Homepage: ');
