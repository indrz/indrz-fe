...
const config$1 = _extends({}, _default);
...
function update (newConfig) {
  ...
  const validKeys = Object.keys(config$1);
...
const asSymbol = function (_ref) {
  ...
  const id = symbol === true ? prefix + '-' + config$1.familyPrefix + '-' + iconName : symbol;
...
function makeInlineSvgAbstract (params) {
  ...
  const attrClass = [config$1.replacementClass, iconName ? config$1.familyPrefix + '-' + iconName : '', widthClass].concat(extra.classes).join(' ');
...
const p = config$1.measurePerformance && PERFORMANCE && PERFORMANCE.mark && PERFORMANCE.measure ? PERFORMANCE : { mark: noop$2, measure: noop$2 };
const preamble = 'FA "5.0.8"';
...
function getCanonicalIcon (values) {
  return values.reduce(function (acc, cls) {
    const iconName = getIconName(config$1.familyPrefix, cls);
...
var mutators = {
  ...
  nest: function nest (mutation) {
    ...
    const forSvg = new RegExp(config$1.familyPrefix + '-.*');
  ...
  disableObservation(function () {
    toArray(root.querySelectorAll('*')).forEach(function (node) {
      [':before', ':after'].forEach(function (pos) {
        ...
        if (pseudoElement) {
          if (pseudoElement.nextSibling && pseudoElement.nextSibling.textContent.includes(DATA_FA_PSEUDO_ELEMENT)) {
        ...
        if (fontFamily && !pseudoElement) {
          ...
          i.textContent = content.length === 3 ? content.substr(1, 1) : content;
          if (pos === ':before') {
...
const css = function () {
  ...
  const fp = config$1.familyPrefix;
